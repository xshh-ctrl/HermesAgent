...
// In dev mode or non-packaged mode, use the source repo root directly.
const ACTIVE_HERMES_ROOT = DEV_SERVER || !IS_PACKAGED ? SOURCE_REPO_ROOT : path.join(HERMES_HOME, 'hermes-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
// In dev mode or non-packaged mode, use .venv (Anaconda-style) or venv.
const VENV_ROOT = DEV_SERVER || !IS_PACKAGED ? path.join(SOURCE_REPO_ROOT, '.venv') : path.join(ACTIVE_HERMES_ROOT, 'venv')
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  const venvRoot = fileExists(path.join(root, '.venv')) ? path.join(root, '.venv') : path.join(root, 'venv')
